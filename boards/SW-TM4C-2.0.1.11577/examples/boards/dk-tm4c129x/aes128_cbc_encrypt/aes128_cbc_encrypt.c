//*****************************************************************************
//
// aes128_cbc_encrypt.c - Simple AES128 cbc encryption demo.
//
...
//! <h1>AES128 CBC Encryption Demo (ae128_cbc_encrypt)</h1>
//!
//! Simple demo showing an encryption operation using the AES128 module in
//! CBC mode.  A number of blocks of data are encrypted.
//!
...
//*****************************************************************************
bool
AES128CBCEncrypt(uint32_t *pui32Src, uint32_t *pui32Dst, uint32_t *pui32Key,
...
//*****************************************************************************
//
// This example encrypts blocks of plaintext using AES128 in CBC mode.  It
...
main(void)
{
    ...
    //
    FrameDraw(&sContext, "aes128-cbc-encrypt");
    ...
    //
    ConfigureUART();
    UARTprintf("Starting AES128 CBC encryption demo.\n");
    ...
    AES128CBCEncrypt(g_pui32AES128PlainText, pui32CipherText, g_pui32AES128Key,
    ...
    for(ui32Idx = 0; ui32Idx < 16; ui32Idx++)
    {
        if(pui32CipherText[ui32Idx] != g_pui32AES128CipherText[ui32Idx])
        {
            UARTprintf("Ciphertext mismatch on word %d. Exp: 0x%x, Act: "
                       "0x%x\n", ui32Idx, g_pui32AES128CipherText[ui32Idx],

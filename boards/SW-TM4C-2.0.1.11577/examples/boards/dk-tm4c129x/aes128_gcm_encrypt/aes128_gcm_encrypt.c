//*****************************************************************************
//
// aes128_gcm_encrypt.c - Simple AES128 GCM encryption demo.
//
...
//! <h1>AES128 GCM Encryption Demo (aes128_gcm_encrypt)</h1>
//!
//! Simple demo showing authenticated encryption operations using the AES
...
//*****************************************************************************
void
AES128HashSubkeyGet(uint32_t *pui32Key, uint32_t *pui32HashSubkey)
{
    uint32_t pui32ZeroArray[4];

    //
    // Put zeroes into the array.
    //
    ...
    // Perform the encryption.
    //
    AES128ECBEncrypt(pui32ZeroArray, pui32HashSubkey, pui32Key, 16);
}
...
//*****************************************************************************
void
AES128GHASH(uint32_t *pui32HashSubkey, uint32_t *pui32IV,
            uint32_t ui32IVLength, uint32_t *pui32Result)
{
    ...
    // Configure the AES module.
    //
    ROM_AESConfigSet(AES_BASE, (AES_CFG_KEY_SIZE_128BIT |
                                AES_CFG_MODE_GCM_HLY0ZERO));

    //
    // Set the hash subkey.
    //
    ROM_AESKey2Set(AES_BASE, pui32HashSubkey, AES_CFG_KEY_SIZE_128BIT);
...
//*****************************************************************************
void
AES128GCMY0Get(uint32_t *pui32IV, uint32_t ui32IVLength, uint32_t *pui32Key,
               uint32_t *pui32Y0)
{
    uint32_t pui32HashSubkey[4];
    ...
    else
    {
        ...
        //
        AES128HashSubkeyGet(pui32Key, pui32HashSubkey);

        //
        // Next, perform the GHASH operation.
        //
        AES128GHASH(pui32HashSubkey, pui32IV, ui32IVLength, pui32Y0);
    }
}

//*****************************************************************************
//
// Perform an GCM encryption operation.
//
//*****************************************************************************
bool
AES128GCMEncrypt(uint32_t *pui32Src, uint32_t *pui32Dst, uint32_t ui32Length,
                 uint32_t *pui32Key, uint32_t *pui32IV, uint32_t *pui32AAD,
                 uint32_t ui32AADLength, uint32_t *pui32Tag, bool bUseDMA)
{
    //
    // Perform a soft reset.
    //
...
main(void)
{
    ...
    //
    FrameDraw(&sContext, "aes128-gcm-encrypt");
    ...
    //
    ConfigureUART();
    UARTprintf("Starting AES128 GCM encryption demo.\n");
        ...
        ui8Vector++)
    {
        ...
        if((ui32DataLength == 0) && (ui32AuthDataLength == 0))
        {
            ...
            //
            AES128GCMY0Get(pui32IV, ui32IVLength, pui32Key, pui32Y0);
        ...
        else
        {
            ...
            //
            UARTprintf("Performing encryption without uDMA.\n");
            AES128GCMEncrypt(pui32PlainText, pui32CipherText, ui32DataLength,
                             pui32Key, pui32Y0, pui32AuthData,

...
WavOpen(const char *pcFileName, tWavFile *psWavData)
{
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, 12, &ui32Count) != FR_OK)
    {
        return(-1);
    }
    ...
    // Look for RIFF tag.
    //
    if((pui32Buffer[0] != RIFF_CHUNK_ID_RIFF) || (pui32Buffer[2] != RIFF_TAG_WAVE))
    {
        return(-1);
    }
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, 8, &ui32Count) != FR_OK)
    {
        return(-1);
    }
    ...
    // Read the next chunk header.
    //
    if(f_read(&psWavData->i16File, pucBuffer, ui32ChunkSize, &ui32Count) != FR_OK)
    {
        return(-1);
    }
...
WavRead(tWavFile *psWavData, unsigned char *pucBuffer, uint32_t ui32Size)
{
    ...
    //
    if(f_read(&psWavData->i16File, pucBuffer, ui32Size, &ui32Count) != FR_OK)
    {
        return(0);
    }

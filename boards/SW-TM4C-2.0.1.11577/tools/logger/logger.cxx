//*****************************************************************************
//
// finder.c - Program to locate Ethernet-based Tiva board on the network.
//
...
void ConstructChannelInfo(void)
{
    ...
    g_ChannelControls[10].pButton = g_pUI->mGyroXSwitch;
    g_ChannelControls[10].pOutput = g_pUI->mGyroXOutput;
    g_ChannelControls[10].pStripChart = g_pUI->mGyroStripChart;
    g_ChannelControls[10].ulMask = 0x00000001;
    g_ChannelControls[10].pcName = "GyroX";
    g_ChannelControls[10].pcUnit = "rad/s";

    g_ChannelControls[11].pButton = g_pUI->mGyroYSwitch;
    g_ChannelControls[11].pOutput = g_pUI->mGyroYOutput;
    g_ChannelControls[11].pStripChart = g_pUI->mGyroStripChart;
    g_ChannelControls[11].ulMask = 0x00000002;
    g_ChannelControls[11].pcName = "GyroY";
    g_ChannelControls[11].pcUnit = "rad/s";

    g_ChannelControls[12].pButton = g_pUI->mGyroZSwitch;
    g_ChannelControls[12].pOutput = g_pUI->mGyroZOutput;
    g_ChannelControls[12].pStripChart = g_pUI->mGyroStripChart;
    g_ChannelControls[12].ulMask = 0x00000004;
    g_ChannelControls[12].pcName = "GyroZ";
    g_ChannelControls[12].pcUnit = "rad/s";

    g_ChannelControls[13].pButton = g_pUI->mMagXSwitch;
    g_ChannelControls[13].pOutput = g_pUI->mMagXOutput;
    g_ChannelControls[13].pStripChart = g_pUI->mMagStripChart;
    g_ChannelControls[13].ulMask = 0x00000001;
    g_ChannelControls[13].pcName = "MagX";
    g_ChannelControls[13].pcUnit = "uT";

    g_ChannelControls[14].pButton = g_pUI->mMagYSwitch;
    g_ChannelControls[14].pOutput = g_pUI->mMagYOutput;
    g_ChannelControls[14].pStripChart = g_pUI->mMagStripChart;
    g_ChannelControls[14].ulMask = 0x00000002;
    g_ChannelControls[14].pcName = "MagY";
    g_ChannelControls[14].pcUnit = "uT";

    g_ChannelControls[15].pButton = g_pUI->mMagZSwitch;
    g_ChannelControls[15].pOutput = g_pUI->mMagZOutput;
    g_ChannelControls[15].pStripChart = g_pUI->mMagStripChart;
    g_ChannelControls[15].ulMask = 0x00000004;
    g_ChannelControls[15].pcName = "MagZ";
    g_ChannelControls[15].pcUnit = "uT";
}
...
void HandleNewPacket(void *pvPacket)
{
    ...
    g_pUI->mCurrentStripChart->AddData(&pPacket->piSamples[9], 1,
                                       (ulLastMask >> 9));
    g_pUI->mGyroStripChart->AddData(&pPacket->piSamples[10], 3,
                                     (ulLastMask >> 10));
    g_pUI->mMagStripChart->AddData(&pPacket->piSamples[13], 3,
                                     (ulLastMask >> 13));
    ...
    g_pUI->mGyroStripChart->damage(FL_DAMAGE_ALL);
    g_pUI->mMagStripChart->damage(FL_DAMAGE_ALL);
...
//*****************************************************************************
//
// Find and open the first Tiva data logger device attached via USB.
//
...
HANDLE OpenLoggerDevice(std::string &strName)
{
    ...
    //
    // The Tiva data logger devices can be identified in this way ...
    // Class - "Ports"
    // FriendlyName - "Stellaris Data Logger Serial Port (COMx)" or
    //                "Tiva Data Logger Serial Port (COMx)" (where x is
    //                1,2,3, etc)
    //
    ...
    // or "Tiva" then "Data Logger Serial Port (COM".
    //
    ulDeviceIndex = 0;
    while(1)
    {
                                            ...
                                            (dwType == REG_SZ))
        {
            ...
            if((_strnicmp(szDesc,
               ...
               (_strnicmp(szDesc,
                         "TivaWare Data Logger Serial Port (COM", 37) == 0))
            {
                //
...
BOOL LoggerDevicePresent(void)
{
    ...
    //
    // Loop through all of the devices in the "Ports" device class,
    // until we find one that has a "Friendly Name" as described above.
    //
    ulDeviceIndex = 0;
    while(1)
    {
                                            ...
                                            (dwType == REG_SZ))
        {
            ...
            if((_strnicmp(szDesc,
                         "Stellaris Data Logger Serial Port (COM", 38) == 0) ||
                (_strnicmp(szDesc,"Tiva Data Logger Serial Port (COM", 33) ==
                 0))
            {
                //
                // Yes - tell the caller.
    ...
    while(1)
    {
        ...
        while(eState != STATE_DISCONNECTED)
        {
            ...
            switch(eState)
            {
                ...
                //
                // We're waiting for the second byte of the packet header.  Did
                // we receive it?
                //
                ...
                case STATE_READ_TIMESTAMP:
                {
                    ...
                    //
                    for(iLoop = 0; iLoop < (sizeof(usMask)*8); iLoop++)
                    {
...
//*****************************************************************************
//
// This program finds Tiva boards that provide the locator service on the
// Ethernet.
//
//*****************************************************************************
int

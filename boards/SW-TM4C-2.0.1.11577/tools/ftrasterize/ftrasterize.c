...
struct option g_pCmdLineOptions[] =
{
    ...
    { "old-width-behavior", no_argument, 0, 'x' },
    { "binary", no_argument, 0, 'y' },
    { "codepage", required_argument, 0, 'z' },
    { 0, 0, 0, 0 }
};

const char * g_pcCmdLineArgs = "a:bc:de:f:g:hilmno:p:rs:t:uvw:xyz:";
...
Usage(char *argv, int bError)
{
    ...
    fprintf(fhOut,
            "  -l            Show the chosen glyphs on the terminal (but don't write any\n"
            "                other output).\n");
    fprintf(fhOut,
    ...
    fprintf(fhOut,
            "  -x            Revert to old (incorrect) font width reporting.  This option\n"
            "                is included for backwards compatibility.  Previous versions of\n"
            "                the tool could report the font width incorrectly in some cases\n"
            "                and it is possible that some applications are relying upon this\n"
            "                behavior.\n");
    fprintf(fhOut,
...
//*****************************************************************************
int
DisplayFontInfo(tConversionParameters *pParams)
{
    FT_Library pLibrary;
    FT_Face pFace;
...
//*****************************************************************************
void
FreeCharMapList(tCodepointBlock *pHead)
{
...
//*****************************************************************************
int
ParseCharMapFile(tConversionParameters *pParams, tCodepointBlock *pHead)
{
    FILE *pFile;
    tCodepointBlock *pBlock, *pNext;
    ...
    while(fgets(pcBuffer, 80, pFile))
    {
        ...
        //
        iNumVals = sscanf(pcBuffer, "%lx, %lx", &ulStart, &ulEnd);
...
//*****************************************************************************
int
SetFontCharSize(FT_Face pFace, tConversionParameters *pParams)
{
    //
...
//*****************************************************************************
unsigned short
CodePageFromEncoding(FT_Encoding eEncoding)
{
    int iLoop;
...
//*****************************************************************************
bool
RenderGlyph(tConversionParameters *pParams, FT_Face pFace,
            unsigned long ulCodePoint, tGlyph *pGlyph, bool bQuiet)
{
        ...
        if(!iError)
        {
            ...
            if(ppMap->width == 0)
            {
                ...
                if(pParams->bVerbose)
                {
                    ...
                    printf("  AdvanceX %ld.%02ld\n",
                           pSlot->metrics.horiAdvance >> 6,
                           pSlot->metrics.horiAdvance & 0x3F);
                    printf("  AdvanceY %ld.%02ld\n",
                           pSlot->metrics.vertAdvance >> 6,
                           pSlot->metrics.vertAdvance & 0x3F);
                }
                ...
                else
                {
                    ...
                    // character as it was defined in the source font.
                    //
...
//*****************************************************************************
bool
DisplayGlyph(tConversionParameters *pParams, tGlyph *pGlyph, int iWidth,
             int iYMin, int iYMax)
{
...
//*****************************************************************************
bool
CompressGlyph(tConversionParameters *pParams, tGlyph *pGlyph, int iWidth,
              int iYMin, int iYMax)
{
...
//*****************************************************************************
void
FreeGlyphs(tGlyph *pGlyph, int iCount)
{
    //
...
//*****************************************************************************
bool
WriteBinaryBlocks(FILE *pFile, tFontInfo *pFont, tGlyph *pGlyphStart,
                  tCodepointBlock *pBlockList)
{
        ...
        iX < pFont->usNumBlocks; iX++)
    {
        ...
        //
        sBlock.ui32StartCodepoint = pBlock->ulStart;
        sBlock.ui32NumCodepoints = (pBlock->ulEnd - pBlock->ulStart) + 1;
        sBlock.ui32GlyphTableOffset = ulOffset;
...
//*****************************************************************************
bool
WriteRemappedBinaryBlocks(FILE *pFile, tFontInfo *pFont, tGlyph *pGlyphStart,
                          tCodepointBlock *pBlockList)
{
    tCodepointBlock *pBlock;
    unsigned long ulOffset, ulTemp;
    ...
    //
    sBlock.ui32GlyphTableOffset = ulOffset;
    sBlock.ui32StartCodepoint = 1;
    sBlock.ui32NumCodepoints = pFont->ulNumGlyphs;
    fwrite(&sBlock, sizeof(tFontBlock), 1, pFile);
...
//*****************************************************************************
bool
WriteBinaryWideFont(tConversionParameters *pParams, tFontInfo *pFont,
                    tGlyph *pGlyphStart, tCodepointBlock *pBlockList)
{
    ...
    //
    sFont.ui8Format = FONT_FMT_WIDE_PIXEL_RLE;
    sFont.ui8MaxWidth = pFont->iWidth;
    sFont.ui8Height = pFont->iYMin - pFont->iYMax + 1;
    sFont.ui8Baseline = pFont->iYMin;
    sFont.ui16Codepage = pFont->usCodePage;
    sFont.ui16NumBlocks = pParams->bRemap ? 1 : pFont->usNumBlocks;
...
//*****************************************************************************
bool
WriteCopyrightBlock(tConversionParameters *pParams, char *pcSize,
...
//*****************************************************************************
bool
WriteRemappedASCIIBlocks(FILE *pFile, tFontInfo *pFont, tGlyph *pGlyphStart,
                         tCodepointBlock *pBlockList)
{
    tCodepointBlock *pBlock;
    ...
    //
    fprintf(pFile, "    //\n");
    fprintf(pFile, "    // Block header 1: Codepoints 0x0001 - 0x%0lx\n",
            pFont->ulNumGlyphs);
    fprintf(pFile, "    //\n");
    ...
    fprintf(pFile, "    //\n");
    fprintf(pFile, "    // Block 1 Offsets: Codepoints 0x0001 - 0x%0lx\n",
            pFont->ulNumGlyphs);
    fprintf(pFile, "    //\n");
    ...
    fprintf(pFile, "\n");
    fprintf(pFile, "    //\n");
    fprintf(pFile, "    // Block 1 Data: Codepoints 1 - 0x%0lx\n",
            pFont->ulNumGlyphs);
    fprintf(pFile, "    //");
...
//*****************************************************************************
bool
WriteASCIIBlocks(FILE *pFile, tFontInfo *pFont, tGlyph *pGlyphStart,
                 tCodepointBlock *pBlockList)
{
    tCodepointBlock *pBlock;
    tGlyph *pGlyph, *pGlyphBlockStart;
        ...
        iX < pFont->usNumBlocks; iX++)
    {
        ...
        //
        fprintf(pFile, "    //\n");
        fprintf(pFile, "    // Block header %d: Codepoints 0x%0lx - 0x%0lx\n",
        ...
        iX < pFont->usNumBlocks; iX++)
    {
        ...
        //
        fprintf(pFile, "    //\n");
        fprintf(pFile, "    // Block %d Offsets: Codepoints 0x%0lx - 0x%0lx\n",
                iX, pBlock->ulStart, pBlock->ulEnd);
        fprintf(pFile, "    //\n");
        ...
        fprintf(pFile, "\n");
        fprintf(pFile, "    //\n");
        fprintf(pFile, "    // Block %d Data: Codepoints 0x%0lx - 0x%0lx\n",
                iX, pBlock->ulStart, pBlock->ulEnd);
        fprintf(pFile, "    //");
...
//*****************************************************************************
bool
WriteASCIIWideFont(tConversionParameters *pParams, tFontInfo *pFont,
                   tGlyph *pGlyphStart, tCodepointBlock *pBlockList)
{
    tCodepointBlock *pBlock;
    tGlyph *pGlyph, *pGlyphBlockStart;
    ...
    fprintf(pFile, "#include <stdbool.h>\n");
    ...
    fprintf(pFile, "const uint8_t g_pui8%s%s%s%s[] =\n{\n", pcCapFilename,
    ...
    //
    fprintf(pFile, "};\n\n");
    fprintf(pFile, "tFont *g_psFontW%s%s%s%s = (tFont *)g_pui8%s%s%s%s;\n",
...
//*****************************************************************************
int
ConvertWideFont(tConversionParameters *pParams)
{
        ...
        while(pBlock)
        {
                ...
                ulCodePoint++)
            {
                   ...
                   !pParams->bNoForceSpace)
                {
                    ...
                    pGlyph[ulGlyphIndex].ulCodePoint = ulCodePoint;
                    pGlyph[ulGlyphIndex].iWidth = 0;
                    pGlyph[ulGlyphIndex].iRows = 0;
                    pGlyph[ulGlyphIndex].iPitch = 0;
                    pGlyph[ulGlyphIndex].iBitmapTop = 0;
            ...
            if(bRetcode)
            {
                if(pGlyph[ulCodePoint].pucChar)
                {
                    printf("Compressed glyph 0x%lx.  Width %d, %d bytes of "
                           ...
                           pGlyph[ulCodePoint].pucChar[0]);

                    //
                    // If the encoded character is wider than the widest we
                    // have yet seen, update our maximum width.  Although we
                    // already set iWidth after rendering the characters,
...
//*****************************************************************************
int
ShowFontCharacters(tConversionParameters *pParams)
{
    tCodepointBlock sBlockListHead;
    tCodepointBlock *pBlock;
    unsigned long ulGlyphIndex, ulCodePoint;
...
//*****************************************************************************
int
ConvertNarrowFont(tConversionParameters *pParams)
{
    ...
    if(iNewStruct)
    {
        ...
        fprintf(pFile, "    //\n");
        fprintf(pFile, "    g_pui16FontOffset%s%d%s%s,\n", pcCapFilename,
                pParams->iSize, pParams->bBold ? "b" : "",
                pParams->bItalic ? "i" : "");
        fprintf(pFile, "\n");
        fprintf(pFile, "    //\n");
...
// compressed raster font for use by the TivaWare Graphics Library.
//
//*****************************************************************************
int
main(int argc, char *argv[])
{
    ...
    sParams.bUseOldWidthBehavior = 0;
    sParams.bVerbose = 0;

    printf("FTRasterize: Generate a TivaWare GrLib-compatible font.\n");
            ...
            g_pCmdLineOptions, &iIndex)) != -1)
    {
        ...
        switch(iOpt)
        {
            ...
            case 'x':
            {
                ...
                //
                sParams.bUseOldWidthBehavior = 1;
